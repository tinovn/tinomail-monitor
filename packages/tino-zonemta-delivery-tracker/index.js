 ...
 * hook callbacks and POSTs them to the TinoMail Monitor backend for ingestion
 * into TimescaleDB.
 *
 * Hooks used:
 ...
 *
 * Events are batched and flushed every FLUSH_INTERVAL_MS or when batch reaches
 * BATCH_SIZE, whichever comes first.
 */
...
module.exports.init = function (app, done) {
    ...
    /** Add event to buffer, auto-flush on batch size */
    ...
    app.addHook('sender:delivered', (delivery, info, next) => {
        try {
            ...
            pushEvent({
                ...
                fromAddress: from.includes('@') ? from : undefined,
    ...
    // Signature: (delivery, connection, err, next)
    app.addHook('sender:responseError', (delivery, connection, err, next) => {
        try {
            ...
            // 5xx = bounced (hard), 4xx = deferred (soft)
    ...
    // --- Hook: bounce (permanent failure, generates bounce email) ---
